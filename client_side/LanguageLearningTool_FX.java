...
public class LanguageLearningTool_FX extends Application {
    @Override
    public void start(Stage primaryStage) {
        ...
        logoImageView.setFitWidth(logoImage.getWidth() * 1.8); // Enlarge the logo
        logoImageView.setFitHeight(logoImage.getHeight() * 1.8);
        ...
        comboBox.setOnAction(e -> {
            ...
            if ("Register".equals(selectedItem)) {
                // Load the JSP page in a WebView
                WebView webView = new WebView();
                WebEngine webEngine = webView.getEngine();
                webEngine.load("http://localhost:8080/Lang_Learning/register");
        
                // Create a new stage for the WebView and show it
                Stage webStage = new Stage();
                webStage.setScene(new Scene(webView, 800, 600));
                webStage.setTitle("Registration Page");
                webStage.show();
